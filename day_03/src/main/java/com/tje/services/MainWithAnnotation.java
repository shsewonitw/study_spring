...
public class MainWithAnnotation {	
	public static void main(String[] args) throws Exception {
		...
		*/
		...
		AbstractApplicationContext ctx = 
				new AnnotationConfigApplicationContext(
						ConnectionProviderConfig.class,
						MemberDAOConfig.class,
						RegistServiceConfig.class,
						SearchingServiceConfig.class);
		// 스프링 컨테이너로부터 커넥션 객체를 생성할 수 있는 객체 반환 
		ConnectionProvider provider = 
				ctx.getBean("provider", ConnectionProvider.class);
		// 스프링 컨테이너로부터 멤버의 중복 여부를 확인할 수 있는 
		// 서비스 객체 반환
		SearchingService ss = 
				ctx.getBean("ss", SearchingService.class);
		// 스프링 컨테이너로부터 멤버를 입력할 수 있는 서비스 객체 반환
		RegistService rs = 
				ctx.getBean("rs", RegistService.class);
